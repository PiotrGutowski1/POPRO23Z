...
WskElListy dodajEl(WskElListy pierwszy, int war) {
    ...
    // Jeśli lista jest pusta, nowy element staje się jej pierwszym elementem
    if (pierwszy == NULL) {
        return nowyElListy;
    }

    /* Założenie: trzeba "od razu" porównywać ze sobą elementy. Są możliwe trzy scenariusze:
     * -> element mniejszy lub równy od pierwszego,
     * -> element "gdzieś w środku" (trzeba się "doczepić" do łańcucha),
     * -> element największy ze wszystkich.
     */
    if(nowyElListy->el <= pierwszy->el) {
        nowyElListy->nast = pierwszy;
        return nowyElListy;
    }

    /* Scenariusz na wypadek, w którym nowy element nie jest ani mniejszy, ani równy pierwszemu. */
    WskElListy aktualny = pierwszy;
    WskElListy poprzedni = NULL;

    while(nowyElListy->el > aktualny->el && aktualny->nast != NULL) {
        poprzedni = aktualny;
        aktualny = aktualny->nast;
    }

    if(nowyElListy->el > aktualny->el) {
        aktualny->nast = nowyElListy;
    } else {
        poprzedni->nast = nowyElListy;
        nowyElListy->nast = aktualny;
    }
